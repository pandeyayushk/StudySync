...
export default async function DashboardLayout({
  ...
  return (
    <div className="min-h-screen bg-transparent flex flex-col">
      ...
      <main className="flex-1 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-20 md:pb-12 text-slate-100">
        {children}
      </main>

      {/* Mobile Bottom Navigation (for small screens) */}
      <nav className="fixed bottom-0 left-0 right-0 z-40 flex h-16 border-t border-slate-800/80 bg-[#080d21]/90 backdrop-blur md:hidden">
        <div className="grid h-full w-full grid-cols-3 mx-auto">
          <Link
            href="/dashboard"
            className="inline-flex flex-col items-center justify-center px-2 hover:bg-slate-800/50 text-slate-400 hover:text-[#15d8b3] transition-colors"
          >
